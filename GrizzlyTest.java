...
public class GrizzlyTest {
	...
	private static final Logger logger = Grizzly.logger(GrizzlyTest.class);
...
//        }
        ...
        final String url = "http://adown.myaora.net:81/test/apkfile/72da6829117a7dcc5a7efd7cb7c34a99.apk";//args[0];
...
class DownloadTask implements Runnable{
	private static final Logger logger = Grizzly.logger(DownloadTask.class);
	String strUrl;
	Connection connection = null;
	Future<Connection> connectFuture = null;
	final FutureImpl<String> completeFuture = SafeFutureImpl.create();
	public DownloadTask(String strUrl){
		this.strUrl = strUrl;
	}
	...
	public void run() {
		...
		try {
	        ...
	        logger.log(Level.INFO, "connect to " + host + " port " + port);
            ...
            connectFuture = transport.connect(host, port);
            try {
